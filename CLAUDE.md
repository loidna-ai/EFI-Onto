...
make missing-audit  # 대화 루프의 결측을 원문으로 되짚는다 — 판독기 탓인가 서식 탓인가
...
- **가설은 아홉이다** — 5대 요인 + 외부화염 + 과부하·과전류 + 누전·지락 + 층간단락 (TTL `[7.7]`~`[7.9]`).
  ...
  '이물 혼입'(표 2-1)도 같은 층이다(D-19: 미확인 단락 ∧ 기기 내 도전물 혼입 확인). 분류표는 단서를 주지 않으므로
  가설로 올리지 않는다 — 금속물 메커니즘은 어떤 가설도 선언하지 않는 확장 슬롯이다.
  **액체는 트래킹, 가루도 트래킹, 물체만 단락이다.** 도전성 액체 유입은 습기 노출의 하위라 트래킹이 선다.
  금속 물체를 트래킹에 이으면 오염 환경 없이 트래킹이 서서 C-5 가 뒤집힌다.
...
- **도출은 읽는 규칙이 있어야 한다.** CONSTRUCT 로 만들기만 하고 아무 규칙·제약도 읽지 않으면 확인해도
  판정에 닿지 못한다. 아크 매핑의 D-4·D-5·D-10 이 그렇게 셋이나 죽어 있었다 — '선언만 되고 안 쓰이는'
  검사가 CONSTRUCT 안의 등장을 사용으로 세었기 때문이다. `make lint` 의 '도출만 되고 읽히지 않는 속성'이 센다.
- **아크 매핑의 '최하류가 최초' 추론은 조건부다 (D-10·D-18·C-59).** 절연전선이든 최하류 지점이든
  '보호장치가 첫 단락에서 동작한다'는 전제 위에 서므로, 그 회로의 과전류 보호 확인과 화재 노출 '국부' 확인이
  같이 있어야 지시력이 선다. 전반 노출이면 전 구간 동시 아크다(Babrauskas p.778). 미확인은 국부가 아니다.
  결론이 `originEvidence` 로 단락흔을 들 때만 검사한다 — 지지 근거로 쓴 단락흔은 발화지점 주장이 아니다.
...
- **질의는 세우는 것부터 묻는다.** 동점이면 동점 전부가 후보이고, 세워지지 않은 가설의 필요조건을 그것이
  ...
  **변별은 규칙이 아니라 지표 단위다** — 같은 지표를 두 후보의 규칙이 다 가리키면 못 가른다(CQ3, `test_competency.py`).
