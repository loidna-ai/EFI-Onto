...
## 부록 D. 사례 150건 도입 이후의 변경 (2026-09)

ABox 가 처음 들어왔다. 그때까지 잰 것은 전부 이식률이었고 정확도는 한 번도 재지
않았다. 재기 시작하자 온톨로지의 결함이 드러났고, 대부분은 **원문과 대조하면 틀린
것**이었다. 정확도는 그것을 찾아준 계기였지 목표가 아니었다.

### D.1 무엇이 틀려 있었나 — 같은 오류의 반복

세 가지 오류가 형태를 바꿔 가며 반복해서 나왔다.

**① 핵심 단서 자리에 원인이 아닌 것이 있었다.**
공식 발화요인 분류는 '반단선에 의한 단락', '압착손상에 의한 단락'이라 적는다.
단락은 공통 귀결이고 변별은 원인에서 나온다. 그런데 압착손상의 핵심이 '외력 직후
즉시 트립'(결과), 접촉불량의 핵심이 '비정상 온도·전압강하'(징후)였다.

**② 상위 개념이 하위의 형제로 놓여 있었다.**
'장기 반복 응력'이 굴곡·진동·인장의 상위가 아니라 형제였다. 조사서는 '굴곡'이라
적지 '장기 반복 응력'이라 적지 않으므로 반단선의 핵심 단서는 한 번도 발동한 적이
없었다. 절연열화도 같았다.

**③ 합성 개념을 요구하고 실무 어휘를 버렸다.**
반단선의 정의('소선 10% 이상 단선')가 규칙에 걸리지 않고 있었다. 우리는
'응력 부위에서 끊어졌는가'라는 복합 사실만 요구했고 조사서는 그 위치를 적지 않는다.

### D.2 편향 점검을 두 축으로 넓혔다

형태 축에는 `audit.py`(화재도 이 흔적을 내는가)가 있었으나 원인 축에는 없었다.
그래서 '원문이 A 도 되고 B 도 된다고 적은 것을 A 쪽으로만 읽는' 오류가 반복됐다.
`cause_audit.py`(정말 그 요인에서만 일어나는가)를 만들어 짝을 맞췄다.

두 축 모두 미검토 0건이 됐고 불변식으로 이관했다. 형태 축에서 여섯 건, 원인 축에서
열한 건이 그 점검으로 교정됐다.

### D.3 가감점을 사례로 검증했다 (보정하지 않았다)

`calibrate.py` 가 같은 공식에 '양립 가능한 가설 수' 대신 '실제로 관측되는 유효
가설 수'(2^H)를 넣어 나란히 놓는다. 21개 중 20개가 5점 이내로 일치했다.
어긋난 것을 따라가 매퍼 결함 셋과 사슬 오류 둘을 찾았다.

보정하지는 않는다. 라벨이 조사관 판정이고, 5요인 균등 표집이며, 150건은 조건부
확률에 적다. 어긋나는 지점은 사슬을 다시 보라는 신호로만 쓴다.

### D.4 천장을 함께 잰다

조사서에 자기 요인을 가리키는 사실이 하나도 없으면 어떤 온톨로지도 못 맞힌다.
그런 사례가 요인마다 크게 달랐다 — 기계손상 0%, 접촉불량 30%, 절연열화 27%.

정확도만 보면 접촉불량 70% 가 부족해 보이지만 상한이 70% 다. 더 할 일은 온톨로지가
아니라 조사서 서식에 있다. 남은 거리를 함께 내지 않으면 이 구분이 보이지 않는다.

그리고 이것은 되짚어 물을 질문을 만든다 — 관측이 없는데 라벨이 붙었다면 무엇으로
판정했는가. NFPA §19.6.5 는 소거법을 과학적 방법으로 인정하지 않는다.

### D.5 두 정확도를 함께 낸다

전체 정확도는 판단보류를 오답으로 센다. 그런데 이 영역은 틀린 답보다 판단보류가
낫고(§19.6.5.1) 온톨로지 전체가 그 원칙 위에 서 있다. 한 숫자로는 그 선택이
보이지 않으므로 '답한 것 중 정확'을 함께 낸다.

### D.6 반증 경로를 채웠다

다섯 요인이 반증 규칙을 하나씩만 갖고 그 지표가 전부 사례 0건이었다. 반증이 한 번도
발동하지 않는 채로 P3(반증 우선)를 구현했다고 적고 있었다. 필요조건의 부재 확인을
반증 규칙으로 물화해 채웠다. ConfirmedAbsent 를 요구하는 규칙이 그때까지 0개였고,
P4(결측·확인불가·부정확인의 구분)도 선언만 하고 쓰지 않고 있었다.

이때 **부재 확인은 포섭 방향이 반대**라는 것이 드러났다. 관측이 '헐거움'이면 상위인
'접속 상태'도 관측된 것이지만, '헐거움 없음'은 '접속 상태 이상 없음'을 뜻하지 않는다.
Python 과 SHACL 양쪽에 같은 규칙을 적었다.

### D.7 국내 실무 지식을 원문에서 이식했다

소방청 화재조사실무Ⅳ(2025) 제2편 전기화재감식. 표 2-1(전기화재 발생 프로세스)이
공식 분류 체계를 담고 있고, 요인을 가르는 구체적 감별 기준이 있었다 — 퓨즈 용단
형태 네 갈래, 반단선 용흔과 절단 용흔의 위치 차이, 반단선의 10% 정의, 아산화동
적색 결정, 자체 발화와 외부 화염의 구별, 절연물 내부 현상(전기 트리·콘덴서 내부
탄화), 층간단락, 절연저항 기준표.

출처는 섞지 않는다. 이 층은 Table 2 와 같은 국내 실무이므로 지표 규칙에 NFPA 조항을
붙이지 않고, 클래스 주석에 쪽수만 남긴다.

### D.8 국내 실무 교재에서 더 이식한 것

**감별 기준** — 퓨즈 용단 형태 네 갈래(단락·과부하·접촉불량·외부화염), 반단선
용흔과 절단 용흔의 위치 차이(양쪽 vs 전원측 한쪽), 반단선의 10% 정의, 아산화동
적색 결정, 이극 간 탄화(트래킹의 정의 그 자체), 절연물 내부 현상(전기·수 트리,
콘덴서 내부 탄화), 층간단락과 그 되먹임 고리.

**보호장치 동작 특성** — §9.6 이 "동작 특성은 없음"으로 남아 있던 자리를 표 2-18
(과전류차단기)·표 2-19(누전차단기, KS C 4613)로 채웠다. 규정 동작시간을 넘겨
과전류가 흘렀는데 동작 기록이 없으면 보호장치 부동작을 도출한다.

**용흔으로 오인되는 것** — 합금 전선(알루미늄·아연)은 용융온도가 낮아 아크 없이도
변색·손상되고 아크를 은폐하므로 용흔 판정법이 적용되지 않는다. 진화·도괴·수습·
조사관 절단으로 생긴 기계적 손상도 용흔과 혼동된다. 둘 다 결론을 막는 제약으로 넣었다.

**어디서 시작됐는가를 세 번 묻는다** — 부품과 주위(§33), 절연물의 표면과 내부(§37),
피복의 안과 밖(§44). 같은 물음의 세 층위이고 셋 다 국부 대비로 답한다.

### D.9 숫자는 규칙에 박지 않는다

절연저항(D-9), 반단선 10%(D-11), 보호장치 동작시간(D-12) 셋 다 국내 기준의
숫자를 갖는다. 그러나 D 규칙은 '임계값이 아니라 관계 비교'라는 원칙이 있으므로,
기준값을 대상의 속성으로 두고 규칙은 잰 값과 그것을 견주기만 한다. 기준이 바뀌면
규칙이 아니라 그 값만 갈아 끼운다. 출처도 분리한다 — 도형은 NFPA 조항을 근거로
달되 숫자가 국내 기준에서 온다는 것을 근거 문장에 명시한다.

### D.10 매퍼는 도구이지 환자가 아니다

정확도가 낮은 곳을 열면 온톨로지가 아니라 매퍼가 틀린 경우가 여러 번 나왔다.
그중 셋은 **온톨로지의 제약을 정확히 우회하는 형태**였다.

- 어떤 차단기 트립이든 '외력 직후 즉시 트립'(압착손상 전용 복합 사실)으로 읽었다.
- 전선 말단의 일반 비드를 '소선 끝단 용융구'(반단선 전용)로 읽었다.
  C-8·§9.11.6 이 "비드만으로는 원인 불가"를 막고 있는데 입력이 그 앞에서 비켰다.
- '부분 정전 현상'을 비통전으로 읽어 전기적 가설 전부를 결정적으로 기각시켰다.

**제약은 입력이 정직할 때만 작동한다.** 실제 시스템에서 LLM 이 판독할 때도 같은
일이 일어날 수 있으므로, 혼동되기 쉬운 클래스에는 주석으로 경계를 적어 두었다.

나머지는 표기 변이였다 — 눌려/굴절/천공/유착/번쩍임처럼 같은 것의 다른 표기.
이런 것을 메우면 정확도와 함께 **상한도 오른다**. 조사서에는 있었는데 우리가 못
읽고 있었다는 뜻이기 때문이다.
