...
## 3단계 · 참조 갱신 — 부분 완료 2026-09-04, 나머지는 다음
...
합치면서 **서로 어긋난 주석**이 나올 것이다. 그것이 이 단계에서 얻는 것이다 — 같은 어휘를 여러 번에 걸쳐 설명하면서 앞뒤가 다르게 적힌 곳이 있고, 한자리에 모이기 전에는 보이지 않았다. 어긋남을 발견하면 원문을 다시 보고 고친다. 주석만 고치는 것이 아니라 사슬이 틀렸을 수도 있다.
