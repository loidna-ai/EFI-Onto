...
누가 받는가 — 표지로 열거된 가설만
  ...
  인스턴스(Table 2 · 국내 실무)가 정한다 — 그 요인의 표지로 열거된 가설이다.
  습기는 트래킹의 필요조건(핵심 단서)이라 트래킹이 받고, 절연열화와도 양립하므로
  k=2 로 감쇠해 9 다. 절연열화는 받지 않는다 — 습기가 절연열화의 표지로 열거된
  적이 없기 때문이다. 양립(사슬)과 표지(규칙)를 섞어 양립하는 가설 전부에 점수를
  주면 필요조건과 단순 가능성이 같은 무게가 되고, 실제로 그렇게 했더니 트래킹이
  96.7% → 10.0% 가 됐다. 규칙이 있는 지표 47개 중 16개가 이 비대칭이며 설계다.
