...
누가 받는가 — 표지로 열거된 가설만
  k 는 '양립하는 가설 수'이고 값의 크기만 정한다. 그 값을 받는 가설은 규칙
  인스턴스(Table 2 · 국내 실무)가 정한다 — 원문이 그 요인의 표지·원인으로 열거한
  가설이다. 사슬(enables)이 양립한다는 것만으로 규칙을 더하지는 않는다.
  다만 '열거됐는가'는 원문 전체를 검색해서 답한다. 습기를 두고 한 번은 p.245 만
  읽고 모터 한정이라 절연열화 규칙을 뺐다가, p.196·201 이 절연열화의 원인 목록
  ①로 든 것을 찾고 되살렸다. 그러면 습기만으로 트래킹과 절연열화는 갈리지 않는다.
  원문이 그렇게 말한다(절연열화의 발화형태가 트래킹, p.196). 가르는 것은 p.149 의
  관례인 대상이며 InterPoleInsulatingSurface 가 그것이다.
