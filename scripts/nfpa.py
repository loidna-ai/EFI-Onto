...
SECTIONS = [
    ...
    ('9.7', 'Branch Circuits', PART, 'Circuit·BranchCircuit·도체 역할과 공급/접지 연결(T01), 접속 금속·도금·설치 맥락(T15)은 구현했다. 허용전류·절연·다발 산정(T16)은 별도다'),
    ('9.8', 'Outlets and Devices', PART, '스위치·극별 연결·내장 보호장치와 T07의 단자 접속 구조·제품별 토크 비교를 구현했다. 정격·허용전류 적용 조건은 T16에 남는다'),
...
SUBSECTIONS = [
    ...
    ("9.7.2",    "AWG 도체 크기와 분기회로 허용전류 표",        PART, "conductorCrossSection_mm2는 있으나 AWG·재질별 표 9.7.2 관계는 없다. "
                                                          "표의 값을 국내 모든 배선에 적용하는 임계값으로 쓰지 않는다"),
    ...
    ("9.7.2.2",  "도체 허용전류는 크기·절연·다발 조건에 따른다",  PART, "D-1은 loadCurrent_A와 ratedCurrent_A를 비교한다. "
                                                          "절연 종류·다발에 따른 허용전류 산정과 차단기 정격의 적합성은 없다"),
    ...
    ('9.8.2', '콘센트의 정격·접지형·극성', PART, '극별 연결·단자 표시·접속 방식과 조건부 토크 비교를 구현(T01·T07). 도체 크기·재질·절연·다발에 따른 정격 적합성은 T16'),
