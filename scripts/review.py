...
sheet('1 단서와 점수',
      ...
      d1, [12, 24, 12, 13, 8, 52],
      '점수(E열)는 예시값이므로 그대로 보지 마시고, 대신 순위(G열)를 매겨 주십시오. '
      '같은 가설 안에서 어느 항목이 더 중요한지 1, 2, 3... 으로 적으시면 됩니다. '
      '숫자가 몇 점이어야 하는지는 답하기 어렵지만 무엇이 더 중요한지는 답할 수 있고, 저희가 순위를 점수로 환산합니다. '
      '노란 줄은 현재 가감점이 커서 결과를 크게 바꾸는 항목입니다.',
      hi_idx=lambda r: abs(r[4]) >= 15, extra_inputs=('순위',))
...
rows = [
    ...
    ('시트 1  단서와 점수', f'{len(d1)}건. 어떤 사실이 어느 가설을 얼마나 지지·반증하는가. '
     '점수는 예시값이므로 점수 자체가 아니라 순위를 매겨 주시면 됩니다. 이 시트가 가장 중요합니다.'),
