...
sheet('2 형태 발현',
      ...
      '노란 줄은 이미 3개 이상의 가설이 공유하는 흔적입니다.',
      hi_idx=lambda r: isinstance(r[2], int) and r[2] >= 3, extra_inputs=('반례 사건번호',))

# ── 3. 선행조건 ──────────────────────────────────────────────────────────
#  조건 하나에 한 줄 (시트 2 와 같은 이유). 통전은 필요조건(C-57)이지 사슬 주장이 아니라 뺀다.
by_ante = {}
for a, b in CH['enables']:
    if a == 'EnergizedState':
        continue
    by_ante.setdefault(a, set()).add(b)
d3 = []
for a in sorted(by_ante, key=lambda x: (len(by_ante[x]), k(x))):
    bs = ' · '.join(sorted(k(b) for b in by_ante[a]))
    d3.append([k(a), bs, len(by_ante[a]),
               f'{k(a)}이(가) 확인되면 {bs}(으)로 이어진다고 되어 있습니다. {k(a)}이(가) 확인됐는데 '
               f'이 밖의 발열로 판정된 사건이 있으면 사건번호를 적어 주십시오.'])
sheet('3 선행조건', ['선행 조건', '이어진다고 된 발열', '발열 수', '문장으로 읽으면'], d3, [22, 40, 8, 52],
      '의견이 아니라 사건을 묻습니다. 그 조건이 있었는데 적힌 것 밖의 발열로 판정된 사건번호 하나가 '
      '찬성 열 개보다 값집니다. 노란 줄은 발열 하나에만 이어진 조건 — 반례 하나로 사슬이 넓어지는 곳입니다.',
      hi_idx=lambda r: r[2] == 1, extra_inputs=('반례 사건번호',))

# ── 3b. 착화 사례 수집 ────────────────────────────────────────────────────
#  반례가 아니라 사례를 모은다. 사례 150건에 착화물 슬롯이 없어 이 축의 근거원은 여기뿐이다.
d3b = []
for a, b in sorted(CH['ignites'], key=lambda x: (k(x[0]), k(x[1]))):
    d3b.append([k(a), k(b),
                f'{k(a)}이(가) {k(b)}에 착화한 것으로 판정된 사건, 또는 착화시키지 못한 사건이 있으면 '
                f'사건번호를 적어 주십시오.'])
sheet('3b 착화 사례 수집', ['발열', '착화물', '문장으로 읽으면'], d3b, [22, 22, 52],
      '이 시트는 반례가 아니라 사례를 모읍니다. 조사서 150건에는 착화물 칸이 없어 이 관계의 근거가 원문뿐입니다. '
      '착화한 사건과 착화하지 못한 사건 모두 값집니다. 판정란은 O = 경험상 맞다, X = 경험상 아니다, △ = 모름.',
      extra_inputs=('착화 사건번호', '미착화 사건번호'))

# ── 4. 현장사실과 증거물 ─────────────────────────────────────────────────
d4 = []
for a, b in sorted(CH['attests'], key=lambda x: (k(x[0]), k(x[1]))):
    d4.append(['현장사실 → 선행조건', k(a), k(b),
               f'{k(a)}이(가) 확인되면 {k(b)}이(가) 있었다고 봅니다. {k(a)}은(는) 확인됐지만 '
               f'{k(b)}은(는) 없었던 사건이 있으면 사건번호를 적어 주십시오.'])
for a, b in sorted(CH['exhibits'], key=lambda x: (k(x[0]), k(x[1]))):
    d4.append(['증거물 → 손상 양상', k(a), k(b),
               f'{k(a)}에서 {k(b)}이(가) 관찰된다고 되어 있습니다. 다른 증거물에서 {k(b)}을(를) '
               f'보신 사건이 있으면 사건번호를 적어 주십시오.'])
sheet('4 현장사실과 증거물', ['관계', '앞', '뒤', '문장으로 읽으면'], d4, [19, 22, 22, 52],
      '의견이 아니라 사건을 묻습니다. 사실은 확인됐는데 조건이 없었던 사건, 다른 증거물에서 같은 흔적이 '
      '나온 사건의 번호를 적어 주십시오.', extra_inputs=('반례 사건번호',))
...
print('rows', len(d1), len(d2), len(d3), len(d3b), len(d4), len(d5), len(d6), len(d7))
