...
for a, b in sorted(CH['enables'], key=lambda x: (k(x[0]), k(x[1]))):
    d3.append(['선행조건 → 발열', k(a), k(b),
               f'{k(a)}은(는) {k(b)}으로 이어진다고 되어 있습니다. {k(a)}이(가) 확인됐는데 '
               f'{k(b)}이(가) 아닌 다른 발열로 판정된 사건이 있으면 사건번호를 적어 주십시오.'])
for a, b in sorted(CH['ignites'], key=lambda x: (k(x[0]), k(x[1]))):
    d3.append(['발열 → 착화물', k(a), k(b),
               f'{k(a)}이(가) {k(b)}에 착화시킨 것으로 판정된 사건, 또는 착화시키지 못한 사건이 있으면 '
               f'사건번호를 적어 주십시오.'])
sheet('3 선행조건과 착화물', ['관계', '앞', '뒤', '문장으로 읽으면'], d3, [17, 22, 22, 52],
      '의견이 아니라 사건을 묻습니다. 그 조건이 있었는데 다른 발열로 판정된 사건번호 하나가 '
      '찬성 열 개보다 값집니다. 착화물 쪽은 실무 판단이 갈리는 부분이라 실제 착화 사례가 특히 필요합니다.',
