...
SUBSECTIONS = [
    ...
    ("11.6", "c", "산화막 저항 — 구리 산화물은 반도체, 알루미늄 산화물은 부도체", DONE, ADDS,
     "T15 MetalInterfaceReference에 산화막 전도 특성과 제안된 금속간화합물 경로를 출처·적용 범위와 함께 보관한다. 접촉불량·발광 접속·가점을 자동 도출하지 않는다. p.550"),
    ...
    ("11.6", "f", "Hagimoto — 교류는 양단 백열, 직류는 +극만. 1 mm 선 16 W·2 mm 선 28 W 일정. 증식 6~18 mm/h. 연선은 끊어져 지속 못 함", DONE, AGREE,
     "실무Ⅳ p.153 '교류 양·음극측, 직류 양극측' 그대로. '연선은 발열점에서 끊어진다'는 접촉불량→반단선 이행의 근거 — ReducedContactArea 주석에 적었다(T34)"),
    ...
    ("14.7", "b", "형광등 안정기 — 권선 단락(shorted turn) 과열이 정상 고장 모드. 화재가 열 차단기를 손상시키므로 안정기를 열어 내부 과열 패턴을 봐야 원인/피해를 가른다", DONE, AGREE,
     "InterTurnShortScenario(실무Ⅳ p.219~220). '내부를 열어 본다'는 InternalDamageExceedingSurface 와 같은 논리. "
     "화재도 권선 용흔·충전재 유출을 내므로 InterTurnArcMarkOnWinding·PottingCompoundLeak 은 외부화염과 공유(audit FIRE)"),
    ...
    ("14.9", "h", "**Ashizawa — PVC 열적 열화 → 염화칼슘 생성(흡습) → 습기 흡수 → 섬광 → 탄화 경로 → 아크 → 착화. 절연저항 1 kΩ 까지 내려간 것만 착화**", PART, CONFLICT,
    ...
    ('14.9', 'j', '콘센트 — 나사 단자 불량이 최다. Meese — 0.3 A 에서도 가시 글로, 129시간 지속, 20 A 에서 20~40 W. **강철 나사가 황동보다 훨씬 취약(자석으로 식별)**', DONE, '보강', 'T07 FastenerRecord에서 실제 단자 나사·재질·자석 반응·식별 근거를 분리한다. 재질 또는 자석 반응에서 글로잉·접촉불량·가점을 자동 도출하지 않는다. p.759'),
    ('14.9', 'k', 'NIST — 14 AWG 구리 나사 최소 토크 0.7 N·m. Hicks — 수명 ∝ 토크³, 권장 1.0~1.6 N·m', DONE, '일치', 'T07 TorqueMeasurement·TorqueReference·TorqueComparison. 제품·접속·도체·측정 종류·시점에 맞는 제조사 기준만 비교하며 0.7 N·m 연구값(p.759)을 공통 문턱으로 쓰지 않는다. p.764의 조건별 시험도 문헌 참고로 구분'),
    ('14.9', 'l', 'back-wired(꽂음식) 콘센트 — 9년 뒤 저항 4배, 98~111℃ vs 나사식 27~28℃. 15%/년 고장', DONE, '보강', 'T07 TerminationRecord가 스프링 삽입식·삽입 후 나사 조임식·나사 단자를 구분한다. p.760의 특정 시험 저항·고장률을 모든 제품의 점수·고장 확률로 쓰지 않는다'),
    ('14.9', 'm', '아무것도 안 꽂힌 콘센트도 출화원이 된다 — 데이지체인 하류 부하 전류가 통과', DONE, '일치', '콘센트를 경유하는 하류 공급 경로를 ConnectionRecord로 연결(T01). 플러그를 표현하지 않은 입력도 경로를 확인하며 플러그 미확인을 반증으로 만들지 않는다'),
    ("14.9", "n", "**Béland — 통전 없이 목재 화재에 노출만 해도 접점이 융착된다. 바이메탈 서모스탯·차단기 접점도. 외부 화재 융착은 부품의 심한 소손으로 가른다**", DONE, CONFLICT,
     "ContactWelding은 FIRE로 정정됐고 ExternalFlameExposure/ExternalFlameScenario에 연결됐다. R_PC_sup8은 공유 수에 따라 재유도한다. CANDIDATES ①는 완료된 변경 기록"),
    ...
    ('14.9', 'r', '알루미늄 배선 문제 (1964~72 미국)', DONE, '—', 'T15: 실제 도체 등급·크기·형태, 접점 모재·도금, 접속·용도·지역·연도·적합 표시를 역사 연구의 명시 범위와 모두 대조한다. 미국 1964~72 소구경 분기배선·구형 기기의 결과를 국내 또는 대형 인입선에 자동 적용하지 않으며 원인 가점을 만들지 않는다. pp.762–765'),
    ...
    ("14.12", "c", "탄화 경로 생성 4경로(UL) — 반복 과전압·고압 트래킹·오염+습기·**열분해(고저항 직렬 고장, 개폐 아크, 기존 화재)**", PART, CONFLICT,
    ...
    ("14.12", "k", "Gross overloads — 드묾. 3~7배 필요. 슬리빙(고무)은 과부하 표지, 열가소성은 구리가 연화 피복에서 '튀어나와' 단락", DONE, AGREE,
     "OverloadScenario(48.1절). Sleeving·MeltOffset·FuseMeltedAtCenter·UniformDamageAlongConductor·OvercurrentFusedBead 가 단서, "
     "D-1 OverloadState 가 핵심, 보호장치 정상 동작 확인이 반증(R_OL_ref2). 정격 맞는 차단기면 과부하 발화 불가(C-34)"),
    ...
    ("14.12", "m", "Stray currents — Kinoshita: PVC 케이블이 아연도금 지붕에 닿으면 5 A 로 착화", DONE, AGREE,
     "GroundFaultScenario·GroundPath(48.2절). 실무Ⅳ p.156·285 지락. 고저항 경로가 발열한다는 점이 GroundFaultHeating 의 정의"),
    ...
    ("14.12", "v", "Overdriven staples — 4경로: 즉시 단락 / **절단+브리지 → 고저항 접속(접촉불량)** / 크리프 / 트래킹. 15시간~수년 지연", DONE, AGREE,
     "MisdrivenStaple은 CD·PC·PD와 ArcTracking에 연결되고 R_TR_sup10이 있다. CANDIDATES ②는 완료된 변경 기록. 못에 녹아 붙은 구리 ≠ 아크(MetalTransferToFastener, §9.11.4.3)"),
    ...
    ("14.15", "e", "수지상 결정 — 있고 없음은 산소 농도만 반영(cause 57%·victim 92% 가 무결정). 팔 간격은 주관적", DONE, CONFLICT,
     "실무Ⅳ p.305 '수지상조직 특징'. fineDendriticGrain 은 대부분의 비드에 적용 불가. 주석에 적었다(T34)"),
...
CANDIDATES = [
    ...
    ("④", "최하류 = 최초 아크 가정의 조건", "강한 화재에 노출된 회로는 전 구간 동시 아크. 부하측이 1차라는 추론은 그때 무효",
     "p.778 (Bernstein)", "구현: D-10 이 노출 국부 확인·과전류 보호 확인을 조건으로 지시력을 세우고, D-18 이 전반 노출을 순서 판정 불가로 표시하며, C-59 이 결론에서 읽는다. 실무Ⅳ 의 규칙은 조건 아래 유지된다"),
    ("⑤", "나사 재질(강철/황동)과 체결 토크", "강철 나사가 황동보다 훨씬 글로잉 취약, 자석으로 식별. 14 AWG 최소 0.7 N·m, 수명 ∝ 토크³",
     "p.759·764 (Meese, NIST, Hicks)", "과거 후보 ⑤는 2026-09-03 T07로 구현. 접속·나사 재질·토크의 출처 있는 입력과 제품 조건 비교를 추가했으며 원인 점수를 자동 생성하지 않는다. 서식 D도 대응 갱신"),
