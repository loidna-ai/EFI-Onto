...
분모의 범위 (T34, 2026-09-04)
  이 온톨로지가 이식한다고 주장하는 원문은 NFPA 921 §4·§9·§19(nfpa.py), 실무Ⅳ 제2편(이 파일),
  Babrauskas 전기 절(babrauskas.py), 그리고 실무Ⅰ·Ⅱ·Ⅴ 중 전기화재 감식 기준을 주는 두 절
  (Ⅰ 제4편 제2장 8. 단락흔에 의한 연소방향, Ⅴ 제3편 제6장 나.⑨~⑫ 차량 전기 결함)이다.
  실무Ⅰ·Ⅱ·Ⅴ 의 나머지 편은 편 단위로 범위밖·현장절차를 적어 '무엇을 안 덮는지' 말할 수 있게 한다.
  세 권의 목차를 2025년판 PDF(docs/references/3-6_*)로 대조했고, 이미 인용하던 쪽(Ⅰ 표 4-9·p.343,
  Ⅱ p.90, Ⅴ p.232~238)이 목차로 봐도 전부였다. 모든 화재조사 문헌의 백로그를 주장하지 않는다.
"""
...
CH = {"1": "제1장 전기화재", "2": "제2장 전기화재 감식",
      ...
      "4": "제4장 전기화재 조사장비",
      # 실무Ⅰ·Ⅱ·Ⅴ (T34). 키는 권-편[.장.절]
      "Ⅰ-1": "실무Ⅰ 제1편 기초화학", "Ⅰ-2": "실무Ⅰ 제2편 기초전기", "Ⅰ-3": "실무Ⅰ 제3편 화재학",
      "Ⅰ-4": "실무Ⅰ 제4편 발화개소판정",
      "Ⅱ-1": "실무Ⅱ 제1편 화재조사총론", "Ⅱ-2": "실무Ⅱ 제2편 보고서 작성요령", "Ⅱ-3": "실무Ⅱ 제3편 조사기구 사용법",
      "Ⅴ-1": "실무Ⅴ 제1편 가스화재감식", "Ⅴ-2": "실무Ⅴ 제2편 폭발화재감식", "Ⅴ-3": "실무Ⅴ 제3편 차량화재감식",
      "Ⅴ-4": "실무Ⅴ 제4편 미소화원감식", "Ⅴ-5": "실무Ⅴ 제5편 방화화재감식"}
...
SECTIONS = [
    ...
    # ── 제2장 전기화재 감식 ──────────────────────────────────────────────────
    ("2.1", 178, "통전입증", PART,
     "'통전 입증부터 시작한다'는 C-57 EnergizedRequiredShape — 통전 확인 없는 전기적 결론을 막는다. "
     "칼날 광택·칼날받이 열림 같은 통전 판별 단서는 어휘가 없다(서식). 하위 절 참조"),
    ...
    # ── 실무Ⅰ·Ⅱ·Ⅴ (2025년판, 중앙소방학교) — 편 단위. 분모 결정은 T34 ───────────
    ("Ⅰ-1", 2, "제1편 기초화학", OUT, "화학 배경 이론. 판정 요건이 없다"),
    ("Ⅰ-2", 78, "제2편 기초전기", OUT,
     "전기 이론과 안전. 제2장 제2절(p.93~)이 누전·단락·과부하를 정의하지만 감식 기준이 아니라 정의다. 실무Ⅳ 1.3 과 같은 판정"),
    ("Ⅰ-3", 110, "제3편 화재학", OUT, "열전달·점화·구획화재 이론. 착화 역량(F-3)은 §19 를 근거로 이미 있다"),
    ("Ⅰ-4", 291, "제4편 발화개소판정", OUT,
     "연소강약·연소방향·화재패턴으로 발화지점을 특정한다. 실무Ⅳ 의 발화개소 절과 같은 판정으로 범위 밖. 8절만 아래에 따로 둔다"),
    ("Ⅰ-4.2.8", 342, "제4편 제2장 8. 단락흔에 의한 연소방향 · 표 4-9", DONE,
     "표 4-9 단락흔·열흔 — FireMeltMark 는 통전 없이 생긴 흔적(45.1)이고 비통전의 증거는 아니다. 부하측이 발화개소측 — furthestDownstream(45.3, D-5). "
     "'구슬모양·광택'은 경향이며 연소열로 사라진다고 원문 스스로 적는다(C-55)"),
    ("Ⅱ-1", 2, "제1편 화재조사총론", OUT,
     "법령·직무. 제2장 제5절(p.45~53)에 화재조사 및 보고규정 본문이 있으나 발화요인 세부분류 별표는 없다. p.11 각주의 훈령 제149호가 T20 판본 근거"),
    ("Ⅱ-2", 82, "제2편 화재조사보고서 작성요령", OUT,
     "보고서 서식과 예시. p.90 은 이미 인용(보호장치 동작 뒤에는 단락흔이 안 생긴다). 예시(p.101·110)의 발화요인 라벨 '전기적 요인 - 절연열화'는 T20 표기 대조 대상"),
    ("Ⅱ-3", 112, "제3편 화재조사기구 사용법", PROC, "장비 제원과 실습. 현장 절차"),
    ("Ⅴ-1", 2, "제1편 가스화재감식", OUT, "가스. 전기적 요인이 아니다"),
    ("Ⅴ-2", 134, "제2편 폭발화재감식", OUT, "폭발. 전기적 요인이 아니다"),
    ("Ⅴ-3", 184, "제3편 차량화재감식", OUT,
     "차량 구조·LPG·사례. 차량 전기 결함 사례(제6장 나.⑨~⑫)만 아래에 따로 둔다"),
    ("Ⅴ-3.6.나", 232, "제3편 제6장 나. 차량 결함 사례 ⑨ 접촉 불량 · ⑩ 스타트 모터 · ⑪ 릴레이 · ⑫ 기판", PART,
     "⑨ p.236 → ReducedContactArea 주석(도체면 반단선, 접속부면 접촉불량). ⑪ p.238 릴레이 접점 융착 → 46.1 접점 융착. "
     "p.232 → C-56 연소 중심과 이격된 단락흔. ⑩ 정류자·브러시 융착과 ⑫ 기판은 차량 부품이라 어휘 없음 — 5대 요인 밖"),
    ("Ⅴ-4", 248, "제4편 미소화원감식", OUT, "무염·훈소 화원. 전기적 요인이 아니다"),
    ("Ⅴ-5", 292, "제5편 방화화재감식", OUT, "방화. 전기적 요인이 아니다"),
]
...
CANDIDATES = [
    ("①", "극별 용융 위치", "한 극만 용융 → 접촉부 과열(접촉불량)·반단선·지락, 양극 용융 → 트래킹",
     "p.185 ①·⑦㉰·⑧", "서식 우선. 조사서 150건에 '한 극'은 0건, '양극'은 1건(UIJEONGBU_2025_055)이고 접속부 용흔을 적은 "
     "28건은 극을 적지 않는다 — 규칙을 붙여도 발동할 사례가 없다. 조사서 서식에 '용융 극' 칸이 먼저(인계 5.3)"),
    ("②", "납땜 불량·균열", "기판 납땜부 접촉저항 증가·크랙 방전",
     "p.185 ⑧, p.222 다㉱, p.237 5)", "서식 우선. 조사서 150건에 납땜은 1건이고 그것도 '납땜 안 하고 꼬아 마감'이다. "
     "자리는 ConnectionCondition 하위이나 사례가 생기기 전엔 만들지 않는다"),
    ("③", "탄화도전로 도통·저항(수Ω~수십Ω)", "트래킹 탄화부를 회로시험기로 재면 도통한다",
     "p.243 (바), p.251 (5), p.272 ③, p.299 (3)", "서식 우선. 조사서 150건에 저항값·도통 확인은 0건. 원문이 네 기기에서 시키는 계측인데 "
     "조사서가 안 적는다. 자리는 CarbonizedConductivePath 의 계측 확인(InsulationResistanceObservation 꼴)"),
    ...
    ("⑦", "통전 입증 단서", "칼날 광택·변색 경계, 칼날받이 열림, 필라멘트 앵커 용착",
     "p.178~179, p.214", "구현 — C-57 EnergizedRequiredShape. EnergizedState 는 143/150 에서 확인으로 읽히고 6개 가설 전부 양립이라 "
     "지지 규칙은 공식상 0점이므로 점수가 아니라 확정의 문턱으로 두었다. 통전 미확인 7건은 조사서가 정말 안 적은 것(62~451자). "
     "칼날 광택·칼날받이 열림 같은 단서 자체는 서식 항목"),
]
...
if __name__ == "__main__":
    n, ins = counts()
    print(f"화재조사실무Ⅳ(2025) 제2편 전기화재감식 + 실무Ⅰ·Ⅱ·Ⅴ 편 단위(T34) — 절 {len(SECTIONS)}개, 범위 내 {ins}개\n")
